...
void    get_combo_2(t_env *env, int **combo_2, int n_combo)
{
    ...
    while (++i < env->nb_valid && (j = i) >= 0)
        while (++j < env->nb_valid)
            if (n_cv + 1 < env->nb_valid * env->nb_valid && paths_compatible(env->paths[i], env->paths[j], env->nb_rooms))
            {
...
void get_combo_x(t_env *env, int **combo_2, int **combo_x, int n_combo)
{
    ...
    while (++i < env->nb_valid && (j = -1))
        while (++j < env->nb_valid && combo_2[j][0] != -1)
            if ((n_cv + 1) < env->nb_valid * env->nb_valid && combo_2[j][0] != -1 && combo_x_compatible(env, env->paths[i], combo_2[j], n_combo))
            {
                combo_x[++n_cv][0] = i;
...
void    combo_optimal(t_env *env)
{
    ...
    if (env->nb_valid > 0)
    {
        ...
        || !(combo_2 = alloc_matrix_int(env->flow_max, env->nb_valid * env->nb_valid, -1))
        || !(combo_x = alloc_matrix_int(env->flow_max, env->nb_valid * env->nb_valid, -1)))
        ...
        free_matrix((void *)combo_2, env->nb_valid * env->nb_valid);
        free_matrix((void *)combo_x, env->nb_valid * env->nb_valid);
    }
    else {
